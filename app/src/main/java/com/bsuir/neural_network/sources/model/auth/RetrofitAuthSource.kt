...
class RetrofitAuthSource(
    ...
    override suspend fun registration(userDTO: LoginUserDTO): Response<HttpResponse> = wrapRetrofitExceptions {
        delay(1000)
        authApi.registration(userDTO)
    }

    override suspend fun login(userDTO: LoginUserDTO): Response<LoginUserAnswerDTO>  = wrapRetrofitExceptions {
        delay(1000)
